...
if is_module_loaded(FILENAME):
    ...
    from RajniiRobot.modules.sql import disable_sql as sql
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "RajniiRobot.modules." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            enable_module = "RajniiRobot.modules." + args[0].rsplit(".", 1)[0]
