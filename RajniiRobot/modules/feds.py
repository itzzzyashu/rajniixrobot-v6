...
import RajniiRobot.modules.sql.feds_sql as sql
from RajniiRobot import (EVENT_LOGS, LOGGER, OWNER_ID, DRAGONS, TIGERS, WOLVES,
                         dispatcher)
...
from RajniiRobot.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                         extract_user,
                                                         extract_user_fban)
from RajniiRobot.modules.helper_funcs.string_handling import markdown_parser
...
def fed_import_bans(update: Update, context: CallbackContext):
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        if fileformat == 'json':
            ...
            text = "Federation is successfully imported fbanned users. {} users are banned.".format(
                success)
            if failed >= 1:
            ...
            if get_fedlog:
                if eval(get_fedlog):
                    teks = "Federatioon *{}* has successfully imported fbanned users. {} users are banned.".format(
            ...
            os.remove("fban_{}.csv".format(
                msg.reply_to_message.document.file_id))
            text = "Files were imported successfully. {} users are banned.".format(
                success)
            if failed >= 1:
                text += " {} Failed to import.".format(failed)
            get_fedlog = sql.get_fed_log(fed_id)
            if get_fedlog:
                if eval(get_fedlog):
                    teks = "Federation *{}* has successfully imported fbanned users. {} users are banned.".format(
                        getfed['fname'], success)
                    if failed >= 1:
                        teks += " {} Failed to import.".format(failed)
                    bot.send_message(get_fedlog, teks, parse_mode="markdown")
...
FED_IMPORTBAN_HANDLER = CommandHandler("fedimport", fed_import_bans)
FEDSTAT_USER = DisableAbleCommandHandler(["fedstat", "fbanstat", "fstat"],
                                         fed_stat_user)
