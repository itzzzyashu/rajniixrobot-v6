...
from RajniiRobot.modules.helper_funcs.extraction import extract_user_and_text
from RajniiRobot.modules.helper_funcs.string_handling import extract_time
from RajniiRobot.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            ...
            message.reply_text(
                "that's one of my wolves, they cannot be banned by me")
        else:
            message.reply_text(
                "Umm this user has immunity and cannot be banned.")
        return log_message
...
def temp_ban(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            # Do not reply
            message.reply_text(
                f"<i>Banned! User will be banned for {time_val}.</i>",
                quote=False,
            )
            return log
        else:
...
PUNCHME_HANDLER = DisableAbleCommandHandler("kickme",
                                            kickme,
